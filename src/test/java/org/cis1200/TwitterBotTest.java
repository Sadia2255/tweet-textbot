package org.cis1200;

import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

import java.time.Duration;
import java.util.LinkedList;
import java.util.List;

/** Tests for TwitterBot class */
public class TwitterBotTest {
    // A helper function for creating lists of strings
    private static List<String> listOfArray(String[] words) {
        List<String> l = new LinkedList<String>();
        for (String s : words) {
            l.add(s);
        }
        return l;
    }

    /*
     * The below test case test whether your TwitterBot class itself is written
     * correctly. To generate a tweet of specific content, make a walk
     * containing the indices of the words you want to appear in the tweet and
     * use that as the {@code NumberGenerator}
     */

    private static final String[] TWEET_1 = { "a", "table", "and", "a", "chair" };
    private static final String[] TWEET_2 = { "a", "banana", "!", "and", "a", "banana", "?" };

    private static List<List<String>> getTestTrainingDataExample() {
        List<List<String>> trainingData = new LinkedList<>();
        trainingData.add(listOfArray(TWEET_1));
        trainingData.add(listOfArray(TWEET_2));
        return trainingData;
    }

    @Test
    public void generatorWorks() {
        TwitterBot tb = new TwitterBot(getTestTrainingDataExample());

        String expected = "a banana?";
        int[] walk = { 0, 0, 1, 0 };
        NumberGenerator ng = new ListNumberGenerator(walk);
        assertEquals(expected, tb.generateTweet(ng));
    }

    /**
     * When the training data is empty, your TwitterBot should create an empty
     * MarkovChain.
     * An empty tweet should be generated by your bot.
     * No exceptions should be thrown and your program should not go into an
     * infinite loop!
     */
    @Test
    public void emptyTrainingDataCreatesEmptyTweet() {
        // Checks that your program does not go into an infinite loop
        assertTimeoutPreemptively(
                Duration.ofSeconds(10), () -> {
                    // No exceptions are thrown if training data is empty
                    TwitterBot tb = new TwitterBot(new LinkedList<>());
                    // Checks that the bot creates an empty tweet
                    assertEquals(0, tb.generateTweet().length());
                }
        );
    }

    /*
     * Add additional test cases here, modeled after the generatorWorks test shown
     * above.
     */

    @Test
    public void testGenerateMultipleTweets() {
        TwitterBot tb = new TwitterBot(getTestTrainingDataExample());
        int numberOfTweets = 5;
        List<String> tweets = tb.generateRandomTweets(numberOfTweets);
        assertEquals(
                numberOfTweets, tweets.size(),
                "The number of tweets generated should match the requested number."
        );
    }

    @Test
    public void testSpecialCharactersAndPunctuation() {
        List<List<String>> trainingData = new LinkedList<>();
        String[] specialCharTokens = { "hello,", "world!", "have", "a", "nice", "day?" };
        trainingData.add(listOfArray(specialCharTokens));
        TwitterBot tb = new TwitterBot(trainingData);

        String expected = "hello, world! have a nice day?";
        int[] walk = { 0, 1, 2, 3, 4, 5 };
        NumberGenerator ng = new ListNumberGenerator(walk);
        assertEquals(
                expected, tb.generateTweet(ng),
                "The tweet should correctly format punctuation and special characters."
        );
    }

    @Test
    public void testRandomTweetGenerationConsistency() {
        List<List<String>> trainingData = getTestTrainingDataExample();
        TwitterBot tb1 = new TwitterBot(trainingData);
        TwitterBot tb2 = new TwitterBot(trainingData);

        RandomNumberGenerator rng1 = new RandomNumberGenerator(123);
        RandomNumberGenerator rng2 = new RandomNumberGenerator(123);

        String tweet1 = tb1.generateTweet(rng1);
        String tweet2 = tb2.generateTweet(rng2);

        assertEquals(
                tweet1, tweet2,
                "Tweets generated with the same seed should be identical."
        );
    }

    @Test
    public void testLongSequenceWalk() {
        String[] longSequence = { "start", "middle", "repeat", "repeat", "repeat", "end" };
        List<List<String>> trainingData = new LinkedList<>();
        trainingData.add(listOfArray(longSequence));
        TwitterBot tb = new TwitterBot(trainingData);

        int[] walkIndices = { 0, 1, 2, 3, 4, 5 };
        NumberGenerator ng = new ListNumberGenerator(walkIndices);

        String expected = "start middle repeat repeat repeat end";
        assertEquals(
                expected, tb.generateTweet(ng),
                "The bot should correctly generate tweets for long sequences."
        );
    }
}